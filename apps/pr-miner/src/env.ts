/**
 * pr-miner の環境変数検証(design.md §9.1 / ADR-0009 / ADR-0013)。
 * 全 AI は Claude on AWS(Agent SDK)。GitHub 認証(gh-client)は PR 読み取りに必要なので、
 * targets 非空なら index.ts が構築する。実 PR 作成は PR_MINER_REAL フラグで guard(既定 dry-run)。
 * KB clone は workflow の checkout に任せ、そのルートを KB_ROOT で受け取る(extractor と異なりアプリ内 clone を持たない)。
...
const envSchema = z
  .object({
    ...
    PR_MINER_REAL: z.string().optional(),
    /** LLM 1 呼び出しのタイムアウト(ms・正の整数)。不正値/未設定は既定 300_000(index.ts で解釈)。 */
    PR_MINER_TIMEOUT_MS: z.string().optional(),
    /** reconcile の並列上限(正の整数)。不正値/未設定は既定 4(index.ts で解釈)。 */
    PR_MINER_RECONCILE_CONCURRENCY: z.string().optional(),
...
export function isRealPr(env: Env): boolean {
  return env.PR_MINER_REAL === "1" || env.PR_MINER_REAL === "true";
}

/**
 * 正の整数の env を解釈する(extractor/env.ts と同一実装のコピー)。
