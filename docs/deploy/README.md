...
> **clones ディレクトリはアプリ別に分離**
> bot・gap-tracker・freshness-checker は KB の clone を**別ディレクトリ**に持ちます:
> bot=`~/stratum/clones`(compose マウント → `/clones`)、gap-tracker=`~/stratum/clones-gap`、
> freshness=`~/stratum/clones-freshness`(unit テンプレの `CLONES_DIR`)。共有していた頃は、
> bot の `/ask` 同期の `git clean -fd` が gap/freshness の validateRepo 前 staging を消し、
> 検証を素通りした未検証 commit が main に入りうる問題があった(2026-07-22)。分離で解消。
> 各 sync は `reset --hard` の後に `git clean -fd` で自分の未追跡 staging 残骸も掃除する
> (これが無いと gap-tracker が自分の dry-run 残骸を「commit 済み」と誤認しキューを消費する)。
> **bot.db だけは共有**(同一 SQLite・WAL + busy_timeout)。既存 VM を移行するときは
> unit を再描画 → `systemctl --user daemon-reload` で新しい `CLONES_DIR` に切り替わる
> (旧 `~/stratum/clones/knowledge-base` の gap/freshness 用コピーは削除してよい)。
