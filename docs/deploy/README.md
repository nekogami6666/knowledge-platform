...
> **本番の bot は KB に url を設定して同期しています(/ask ごとに fetch+reset+clean)**。そのため
> gap-tracker / freshness が validateRepo 用に staging している最中に bot の clean がそれを消すと、
> 検証を素通りした未検証 commit が main に入りうる(発生窓は staging→validate の数秒)。
> 恒久対応 = **アプリ別 CLONES_DIR 分離**(unit テンプレの CLONES_DIR を gap/freshness 個別
> ディレクトリへ)を follow-up で行う。
