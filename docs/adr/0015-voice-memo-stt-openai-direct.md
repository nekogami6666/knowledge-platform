...
- **備考**: 採択(`accepted`)および design.md への転記(§14#1 決定済み化・§6.4 L484 の経路改訂・
  §9.4 の外部送信先確定・§8.3 プロンプト表)は人間レビューで行う。
...
サイクルの Python システムであり、結合は双方の変更自由度を損なう)。L483 のフォールバック条項を適用する
(文言上の「Whisper API」は同じ OpenAI transcription API 系列の現行モデルと読み替え、design 転記時に文言を更新)。
...
- 実装着手前に OpenAI のデータ利用ポリシー(学習利用・保持期間)を公式ドキュメントで確認し、
  確認日付と内容を本 ADR に追記する(ADR-0002 と同じ運用)。
