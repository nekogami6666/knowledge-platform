...
> **追記(2026-08-20)**: 同じ機構でエントリ数上限 `EXTRACTOR_MAX_ENTRIES` を追加した。ファイル上限
> (5 件)でも 1 PR に新規 24〜45 エントリが出てレビューが滞留したため、「1 晩でレビューし切れる量」
> をエントリ粒度で有界化する(CI は 20)。判定は**ファイル境界のみ**(1 ファイルは不可分に処理し
> 多少の超過を許す — pr-miner の cap と同じ思想)。持ち越しは本 ADR の `pending` をそのまま使う。
