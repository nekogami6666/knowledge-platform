...
- すべてのバッチは**再実行安全**であること。カーソル(`_meta/state.json`)更新は処理成功後のみ。途中失敗 → 再実行で重複 PR が立たないよう、**未マージの提案 PR をブランチ名の prefix(`extract/` / `pr-miner/`)で検出し、1 本でも open なら次の run を見送る**。カーソルはマージ時にしか main へ反映されないため、未マージのまま再実行しても同じ範囲を再抽出するだけで PR が積み上がる(処理範囲を PR タイトルの SHA range で判定していた頃は、上流リポに新コミットが入るとキーが変わってガードが外れた)。PR タイトルには引き続き処理範囲を残し、人が読んで判別できるようにする
